<template>
  <div class="woreda-allocation-form-container">
    <el-page-header @back="handleBack" :title="$t('common.back')">
      <template #content>
        <span>{{ isEdit ? $t('allocation.editWoredaAllocation') : $t('allocation.addWoredaAllocation') }}</span>
      </template>
    </el-page-header>
    ...
    <el-card v-loading="loading" class="form-card">
      <el-form :model="formData" :rules="rules" ref="formRef" label-width="150px">
        ...
        <el-form-item :label="$t('allocation.woreda')" prop="zone">
  <el-select 
...
import { getWoredaAllocationDetail, addWoredaAllocation, editWoredaAllocation } from '@/api/allocation'
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const apiFunc = isEdit.value ? editWoredaAllocation : addWoredaAllocation
...
<style scoped>
.woreda-allocation-form-container {
  padding: 20px;
}
.form-card {
  margin-top: 20px;
