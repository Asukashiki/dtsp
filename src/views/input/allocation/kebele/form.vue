<template>
  <div class="kebele-allocation-form-container">
    <el-page-header @back="handleBack" :title="$t('common.back')">
      <template #content>
        <span>{{ isEdit ? $t('allocation.editKebeleAllocation') : $t('allocation.addKebeleAllocation') }}</span>
      </template>
    </el-page-header>
    ...
    <el-card v-loading="loading" class="form-card">
      <el-form :model="formData" :rules="rules" ref="formRef" label-width="150px">
        ...
        <el-form-item :label="$t('allocation.kebele')" prop="zone">
  <el-select 
...
import { getKebeleAllocationDetail, addKebeleAllocation, editKebeleAllocation } from '@/api/allocation'
...
const fetchDetail = async () => {
  loading.value = true
  try {
    const response = await getKebeleAllocationDetail(route.params.id)
    if (response.code === 200 && response.data) {
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      const apiFunc = isEdit.value ? editKebeleAllocation : addKebeleAllocation
...
<style scoped>
.kebele-allocation-form-container {
  padding: 20px;
}
.form-card {
  margin-top: 20px;
