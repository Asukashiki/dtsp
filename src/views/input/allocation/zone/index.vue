  ...
  <div class="page-container">
    <div class="page-wrapper">
      ...
      <PageHeader
        ...
        :title="$t('allocation.menu.zoneAllocation')"
        :subtitle="$t('allocation.menu.zoneAllocation')"
      />
      ...
      <div class="content-wrapper">
        ...
        <!-- 数据列表 -->
        <InfoCard :title="$t('allocation.menu.zoneAllocation')" icon="ri-file-list-3-line">
          <template #actions>
          ...
          <div class="table-wrapper pc-only">
            <el-table :data="allocationList" v-loading="loading" border stripe>
              ...
              <el-table-column prop="zoneName" :label="$t('allocation.zone')" min-width="150" />
          ...
          <div class="mobile-card-list mobile-only">
            <div v-for="item in allocationList" :key="item.id" class="mobile-card">
              ...
              <div class="mobile-card-body">
                ...
                <div class="mobile-card-row">
                  <span class="label">{{ $t('allocation.zone') }}:</span>
...
const handleQuery = async () => {
  ...
  try {
    // 调用API获取列表数据
    const response = await getZoneAllocationList(queryParams)
...
const handleAdd = () => {
  router.push('/input/allocation/zone/add')
}

const handleEdit = (row) => {
  router.push(`/input/allocation/zone/edit/${row.id}`)
}

const handleView = (row) => {
  router.push(`/input/allocation/zone/detail/${row.id}`)
}

const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(t('common.confirmDelete'), t('common.warning'), { type: 'warning' })
    const response = await deleteZoneAllocation(row.id)
    if (response.code === 200) {
