      ...
      <div class="adjustment-workspace">
        <div class="detail-table-panel">
          ...
          >
            <el-table-column
              type="index"
              :label="$t('townAggregationAudit.adjustment.index')"
              width="60"
              fixed="left"
            />
            <el-table-column
              prop="sourceName"
              :label="$t('townAggregationAudit.adjustment.kebeleName')"
              min-width="150"
              fixed="left"
            />
            <el-table-column
              prop="inputType"
              :label="$t('townAggregationAudit.adjustment.inputType')"
              min-width="130"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              prop="inputCategory"
              :label="$t('townAggregationAudit.adjustment.category')"
              min-width="140"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              prop="variety"
              :label="$t('townAggregationAudit.adjustment.variety')"
              min-width="130"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              prop="season"
              :label="$t('townAggregationAudit.adjustment.season')"
              min-width="130"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              ...
              :label="$t('townAggregationAudit.adjustment.receivedDemand')"
              min-width="170"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              ...
              :label="$t('townAggregationAudit.adjustment.adjustedDemand')"
              min-width="170"
            >
              <template #default="{ row }">
            ...
            <el-table-column
              ...
              :label="$t('townAggregationAudit.adjustment.adjustmentRemark')"
              min-width="220"
            ...
            <el-table-column
              ...
              :label="$t('townAggregationAudit.adjustment.status')"
              min-width="120"
            >
              <template #default="{ row }">
            ...
            >
              <template #default="{ row }">
                <div class="detail-actions">
                  ...
                  >
                    {{ $t('townAggregationAudit.adjustment.adjust') }}
                  </el-button>
                  <!-- Submit to Zone is temporarily hidden per business flow adjustment. -->
                  <!--
                    <el-button
                      size="small"
                      type="success"
                    ...
                    >
                      {{ row.adjustmentStatus === 'submitted'
                        ? $t('townAggregationAudit.adjustment.alreadySubmitted')
                        : $t('townAggregationAudit.adjustment.submitToZone') }}
                    </el-button>
                  -->
                  <el-button
                    size="small"
                  ...
                  >
                    {{ $t('townAggregationAudit.adjustment.viewHistory') }}
                  </el-button>
                </div>
              </template>
            </el-table-column>
        ...
        <aside class="adjustment-side-panel">
          <el-empty
            ...
            :description="$t('townAggregationAudit.adjustment.emptyTitle')"
          >
            <template #description>
              <p>{{ $t('townAggregationAudit.adjustment.emptyTip') }}</p>
            </template>
          </el-empty>

          <div v-else-if="sidePanelMode === 'adjust' && selectedDetailRow" class="side-content">
            <div class="side-header">
              <h3>{{ $t('townAggregationAudit.adjustment.adjustPanelTitle') }}</h3>
            ...
            <div class="detail-summary">
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.kebeleName') }}</span>
                <strong>{{ selectedDetailRow.sourceName || '-' }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.inputType') }}</span>
                <strong>{{ getDictLabel('input_type', selectedDetailRow.inputType) }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.category') }}</span>
                <strong>{{ getDictLabel('input_category', selectedDetailRow.inputCategory) }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.variety') }}</span>
                <strong>{{ selectedDetailRow.variety || selectedDetailRow.varieties || '-' }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.season') }}</span>
                <strong>{{ getSeasonLabel(selectedDetailRow.season) }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.receivedDemand') }}</span>
                <strong>{{ formatQuantity(selectedDetailRow.receivedQuantity, selectedDetailRow.unit || selectedDetailRow.units) }}</strong>
              </div>
              <div>
                <span>{{ $t('townAggregationAudit.adjustment.currentAdjustedDemand') }}</span>
            ...
            >
              <el-form-item
                :label="$t('townAggregationAudit.adjustment.newAdjustedDemand')"
                prop="adjustedQuantity"
              >
                <el-input-number
            ...
            <div class="side-actions">
              <el-button @click="resetSidePanel">
                {{ $t('townAggregationAudit.adjustment.cancel') }}
              </el-button>
              <el-button
                type="primary"
              ...
              >
                {{ $t('townAggregationAudit.adjustment.saveAdjustment') }}
              </el-button>
            </div>
          </div>
          ...
          <div v-else-if="sidePanelMode === 'history' && selectedDetailRow" class="side-content">
            <div class="side-header">
              <h3>{{ $t('townAggregationAudit.adjustment.historyTitle') }}</h3>
              <el-button
            ...
            >
              <el-table-column
                prop="createdTime"
                :label="$t('townAggregationAudit.adjustment.operationTime')"
                min-width="150"
              />
              <el-table-column
                prop="beforeQuantity"
                :label="$t('townAggregationAudit.adjustment.beforeQuantity')"
                min-width="110"
              />
              <el-table-column
                prop="afterQuantity"
                :label="$t('townAggregationAudit.adjustment.afterQuantity')"
                min-width="110"
              />
              <el-table-column
                prop="operatorName"
                :label="$t('townAggregationAudit.adjustment.operator')"
                min-width="110"
              >
                <template #default="{ row }">
                  {{ row.operatorName || '-' }}
              ...
              <el-table-column
                prop="remark"
                :label="$t('townAggregationAudit.adjustment.remark')"
                min-width="180"
                show-overflow-tooltip
              />
            </el-table>
            <el-empty
              ...
              :description="$t('townAggregationAudit.adjustment.noHistory')"
            />
          </div>
        </aside>
      </div>
...
const adjustRules = reactive({
  adjustedQuantity: [
    { required: true, message: t('townAggregationAudit.adjustment.requiredAdjustedQuantity'), trigger: 'blur' }
  ],
  adjustmentRemark: [
    { required: true, message: t('townAggregationAudit.adjustment.requiredAdjustmentRemark'), trigger: 'blur' }
  ]
})
...
const getAdjustmentStatusLabel = (status) => {
  ...
  return t(`townAggregationAudit.adjustmentStatus.${key}`)
}
...
const loadData = async () => {
  loading.value = true
  try {
    const countyCode = getStoredUserInfo().deptId
    const params = {
...
const handleSaveAdjustment = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      ElMessage.success(t('townAggregationAudit.adjustment.saveSuccess'))
    } else {
      ElMessage.error(res.msg || t('townAggregationAudit.adjustment.saveFailed'))
    }
  } catch (error) {
    console.error('Failed to save adjustment:', error)
    ElMessage.error(t('townAggregationAudit.adjustment.saveFailed'))
  } finally {
...
const handleViewHistory = async (row) => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      ElMessage.error(res.msg || t('townAggregationAudit.adjustment.historyLoadFailed'))
    }
  } catch (error) {
    console.error('Failed to load adjustment history:', error)
    ElMessage.error(t('townAggregationAudit.adjustment.historyLoadFailed'))
  } finally {
...
const handleSubmitToZone = async (row) => {
  try {
    await ElMessageBox.confirm(
      t('townAggregationAudit.adjustment.submitConfirm'),
      t('common.warning'),
      {
    ...
    if (res.code === 200) {
      ...
      ElMessage.success(t('townAggregationAudit.adjustment.submitSuccess'))
    } else {
      ElMessage.error(res.msg || t('townAggregationAudit.adjustment.submitFailed'))
    }
  } catch (error) {
    if (error !== 'cancel') {
      console.error('Failed to submit detail to Zone:', error)
      ElMessage.error(t('townAggregationAudit.adjustment.submitFailed'))
    }
  } finally {
