        ...
        <el-form ref="formRef" :model="formData" :rules="rules" label-width="140px" v-loading="loading">
          <div class="info-card">
            ...
            <div class="card-body">
              <el-row :gutter="20">
                ...
                <el-col :xs="24" :sm="12">
                  <el-form-item label="source_organization">
                    <el-input v-model="formData.source_organization" disabled />
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="12">
                ...
                <el-col :xs="24" :sm="12">
                  <el-form-item label="allocation_type">
                    <el-select v-model="formData.allocation_type" :placeholder="$t('common.pleaseSelect')" style="width: 100%">
                      <el-option v-for="item in allocationTypeOptions" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="12">
                  <el-form-item label="approved_by">
                    <el-input v-model="formData.approved_by" :placeholder="$t('common.pleaseInput')" />
                  </el-form-item>
                </el-col>
                <el-col :xs="24" :sm="12">
                  <el-form-item label="approval_date">
                    <el-date-picker v-model="formData.approval_date" type="date" value-format="YYYY-MM-DD" style="width: 100%" />
                  </el-form-item>
                </el-col>
              </el-row>

              <el-table
                v-if="formData.zoneId"
                :data="demandList"
                border
          ...
          <div class="info-card">
            ...
            <div class="card-body">
              <el-row :gutter="20">
                <el-col :xs="24" :sm="24">
                  <el-form-item label="minute_file">
                    <el-upload
                      v-model:file-list="minuteFileList"
                      :action="uploadUrl"
                      :headers="uploadHeaders"
                      :before-upload="beforeMinuteFileUpload"
                      :on-success="handleMinuteFileUploadSuccess"
                      :on-remove="handleMinuteFileRemove"
                      :on-exceed="handleMinuteFileExceed"
                      :limit="1"
                      list-type="text">
...
import { getBoaZoneReleaseDetail, addBoaZoneRelease, editBoaZoneRelease, getDeptCategoryStock } from '@/api/inputCirculation'
...
const formData = reactive({
  ...
  source_organization: 'Oromia Bureau of Agriculture',
  allocation_type: '',
  approved_by: '',
  approval_date: '',
  minute_file: '',
  ...
  auditBy: '',
  releaseOrg: '',
  releaseType: 'BOA_TO_ZONE', // OSE?????nion
  details: []
})
...
const syncMinuteFileList = () => {
  if (!formData.minute_file) {
    minuteFileList.value = []
    return
  }
  minuteFileList.value = [{
    name: formData.minute_file_name || getFileNameFromPath(formData.minute_file),
    url: formData.minute_file
  }]
}

const beforeMinuteFileUpload = (file) => {
  const isLt20M = file.size / 1024 / 1024 < 20
  if (!isLt20M) {
    ElMessage.error('Attachment size cannot exceed 20MB')
    return false
  }
  return true
}

const handleMinuteFileUploadSuccess = (response, uploadFile) => {
  ...
  const fileUrl = uploadData.url || uploadData.path || (typeof uploadData === 'string' ? uploadData : '')
  if (!fileUrl) {
    ElMessage.error(t('common.uploadFailed'))
    return
  }

  formData.minute_file = fileUrl
  formData.minute_file_name = uploadData.name || uploadData.fileName || uploadFile?.name || getFileNameFromPath(fileUrl)
  syncMinuteFileList()
  ElMessage.success(t('common.uploadSuccess'))
}

const handleMinuteFileRemove = () => {
  formData.minute_file = ''
  formData.minute_file_name = ''
  minuteFileList.value = []
}

const handleMinuteFileExceed = () => {
  ElMessage.warning('Only one attachment is allowed')
}
...
const handleSeasonChange = async (index) => {
  const detail = formData.details[index]
  if (!detail) return
  detail.maxQuantity = getDemandQuantity(detail.inputType, detail.inputCategory, detail.variety, detail.season)
  await validateQuantity(index)
}
...
const fetchDetail = async () => {
  ...
  try {
    ...
    if (response.code === 200 && response.data) {
      ...
      formData.source_organization = formData.source_organization || 'Oromia Bureau of Agriculture'
      syncMinuteFileList()
...
const handleSubmit = async () => {
  ...
  await formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      const apiFunc = isEdit.value ? editBoaZoneRelease : addBoaZoneRelease
