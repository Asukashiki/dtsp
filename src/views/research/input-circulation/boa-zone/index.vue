  ...
  <div class="page-container">
    <div class="page-wrapper">
      ...
      <PageHeader
        ...
        :title="$t('inputCirculation.boaToZone')"
        :subtitle="$t('inputCirculation.boaToZone')" />
      ...
      <div class="content-wrapper">
        ...
        <!-- 列表卡片 -->
        <InfoCard :title="$t('inputCirculation.boaToZone')" icon="ri-file-list-3-line">
          <template #actions>
...
import { getBoaZoneReleaseList, deleteBoaZoneRelease, getBoaZoneReleaseStockStatus } from '@/api/inputCirculation'
...
const handleQuery = async () => {
  loading.value = true
  queryParams.releaseType = 'BOA_TO_ZONE'
  ...
  try {
    const response = await getBoaZoneReleaseList(queryParams)
...
const loadStockStatus = async () => {
  ...
  try {
    const response = await getBoaZoneReleaseStockStatus(releaseIds)
...
const handleAdd = () => {
  router.push('/input/input-circulation/boa-zone/add')
}

const handleEdit = (row) => {
  router.push(`/input/input-circulation/boa-zone/edit/${row.id}`)
}

const handleView = (row) => {
  router.push(`/input/input-circulation/boa-zone/detail/${row.id}`)
}

const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(t('common.confirmDelete'), t('common.warning'), { type: 'warning' })
    const response = await deleteBoaZoneRelease(row.id)
    if (response.code === 200) {
...
const handleDeleteBatch = async () => {
  ...
  try {
    await ElMessageBox.confirm(t('common.confirmDelete'), t('common.warning'), { type: 'warning' })
    const response = await deleteBoaZoneRelease(selectedIds.value.join(','))
    if (response.code === 200) {
      ElMessage.success(t('common.deleteSuccess'))
      handleQuery()
    } else {
